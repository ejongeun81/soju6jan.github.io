...
----------------------------------------------------------------------------
2020-08-19
나스당에 php 크롤링 글이 올라왔고, 예전 코드도 돌아가는지 호김심에 확인해봤습니다.
크롤링은 변경사항 없이 그대로 동작했고, filetender 부분은 약간 수정하니 동작합니다.
발견하여 사용할 사람은 없겠지만 수정한 김에 올립니다.
filetender는 아마 크롤링 방지 기능이 있어서 자주 요청하면 차단당할 확률이 높을 겁니다.
*/
...
function download() {
    ...
    preg_match("/newUrl\s\=\s\'(?P<url>.*?)\'\;/", $data, $matches);
    $newUrl = $matches['url'];
    preg_match_all('/name=\"(?P<name>\w+)\"\svalue\=\"(?P<value>.*?)\"/', $data, $matches);
    $param = '';
    for($x = 0 ; $x < count($matches['name']) ; $x++) {
        $param .= $matches['name'][$x].'='.$matches['value'][$x];
        if ($x != count($matches['name']) -1) $param .= '&';
	}

    $url3 = $newUrl.'?'.$param;
    $headers[0] = 'Referer: '.$url2;
    $headers[4] = 'Host: file.filetender.com';
